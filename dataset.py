...
class DataCollatorForDialougeDenoising:
        ...
        for i in range(bsz):
            ...
            while num_to_mask != 0:
                ...
                if span_length != 0:
                    ...
                    inputs[i] = np.concatenate([np.delete(inputs[i], span), [self.tokenizer.pad_token_id] * span_length])
                else : 
                    while inputs[i][span_start] in special_ids and inputs[i][span_start-1] in special_ids:
                        span_start = np.random.randint(1, window_length-span_length)
    ...
    def turn_permutation(self, inputs):
        ...
        for i in range(bsz):
            ...
            turn_list = [(speaker_idx, sep_idx) for speaker_idx, sep_idx in zip(speaker_indices, sep_indices)]   
            ...
            if len(speaker_indices) > 1:
                inputs_copy = np.insert(inputs_copy, speaker_indices[1:], self.tokenizer.sep_token_id)

            if length_before == len(inputs_copy)+1:
                inputs_copy = np.insert(inputs_copy, -1, self.tokenizer.sep_token_id)
                
            assert length_before == len(inputs_copy), f"{length_before}, {len(inputs_copy)}, \n{inputs[i][:length_before]} \n{inputs_copy} \n{self.tokenizer.decode(inputs_copy)} \n{self.tokenizer.decode(inputs[i][:length_before])}, {self.tokenizer.decode(self.speaker_ids)}"
            
            inputs[i] = np.concatenate([inputs_copy, [self.tokenizer.pad_token_id] * np.count_nonzero(pad_indices)])
    ...
    def make_labels(self, inputs):
        ...
        eos_indices = [
            max_length - pad_idx.count(True)
            if pad_idx.count(True) != 0 else max_length -1
